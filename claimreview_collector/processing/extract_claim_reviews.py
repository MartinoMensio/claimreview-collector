...
def extract_ifcn_claimreviews(domains=None, recollect=True, unshorten=True):
    ...
    if recollect:
        ...
        with ThreadPool(8) as pool:
            for url, crs in tqdm.tqdm(
                pool.imap_unordered(
                    claimreview.retrieve_claimreview, urls_to_recollect
                ),
                total=len(urls_to_recollect),
                desc="recollecting",
            ):
                for cr in crs:
        ...
        utils.write_json_with_path(
            recollected_crs, data_path, "claim_reviews_raw_recollected.json"
        )
            ...
            results.append(
                {
                    ...
                    "review_url": url,
                    "fact_checker": {
                        "name": factchecker_info["original"].get("name"),
                        "country": factchecker_info["original"].get("country"),
                        "language": factchecker_info["original"].get("language"),
                        "website": factchecker_info["original"].get("website"),
                        "ifcn_url": factchecker_info["original"].get("assessment_url"),
                        "avatar": factchecker_info["original"].get("avatar"),
    ...
    # 10190
    utils.write_json_with_path(
        {k: list(v) for k, v in different_texts.items()},
        data_path,
        "different_texts.json",
    )
    ...
    utils.write_json_with_path(
        disagreeing_reviews, data_path, "disagreeing_reviews.json"
    )
...
def cluster_sentences(sentences, max_distance=3):
    ...
    # indexes
    filtered_clusters = [[el] for el in range(len(sentences))] + [[]] * (
        len(sentences) - 1
    )
