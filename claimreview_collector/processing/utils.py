...
def aggregate(data_list, key="url"):
    ...
    by_key = {
        k: list(v)
        for k, v in itertools.groupby(sorted(data_list, key=by_key_fn), key=by_key_fn)
    }
    agree = {}
    not_agree = {}
    for k, k_group in by_key.items():
        ...
        by_label = {
            k: [el["source"] for el in v]
            for k, v in itertools.groupby(
                sorted(k_group, key=by_label_fn), key=by_label_fn
            )
        }
...
def compute_by_domain(url_based_data, decision_mode="all_agree"):
    ...
    by_domain = {
        k: list(v)
        for k, v in itertools.groupby(
            sorted(url_based_data, key=by_domain_fn), key=by_domain_fn
        )
    }
    ...
    if decision_mode == "all_agree":
        for k, v in by_domain.items():
            ...
            if len(label_set) != 1:
                ...
                label = label_set.pop()
                result.append({"domain": k, "label": label, "source": v[0]["source"]})
    else:
