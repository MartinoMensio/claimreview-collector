...
def load_jsonld(content):
    ...
    labels_by_sources = {
        ...
        for k, v in itertools.groupby(
            sorted(claimReviews, key=lambda_source), key=lambda_source
        )
    }
    ...
    # print('labels', labels)
    print(
        "#different labels for each source",
        {k: len(v) for k, v in labels_by_sources.items()},
    )
...
def enrich_claimReviews(claim_reviews):
    ...
    with ThreadPool(8) as pool:
        ...
        for one_result in tqdm.tqdm(
            pool.imap_unordered(claimreview.retrieve_claimreview, urls), total=len(urls)
        ):
