...
class Scraper(ScraperBase):
    ...
    def scrape(self, update=True):
        if update:
            es_reviews = scrape_all(
                self.id,
                "https://spreadsheets.google.com/feeds/list/1mMf5y2_CvlbTCzgE6EA6ZhFR7RAvTXhd1713kUpIPOw/od6/public/values?alt=json",
            )
            pt_reviews = scrape_all(
                self.id,
                "https://spreadsheets.google.com/feeds/list/1BIgVzAcMhWXR5oW9c1UB2vnW5RY6vni_FRXvBaAG_4Q/od6/public/values?alt=json",
            )
        ...
        for r in tqdm.tqdm(all_reviews):
            try:
                ...
                url_fixed, cr = claimreview.retrieve_claimreview(
                    r.get("Link al chequeo", r.get("Link para a checagem"))
                )
