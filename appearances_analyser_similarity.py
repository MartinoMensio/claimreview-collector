...
text_claims = [el['claimReviewed'] if 'claimReviewed' in el else '' for el in claim_reviews]
...
if os.path.exists(path_claim_embedding):
    ...
    for b in tqdm(get_batch(text_claims, batch_size), total=len(text_claims)//batch_size):
        # print(b)
        embeddings = embed(b)
...
for i, step in tqdm.tqdm(enumerate(Z), desc='running truncated hierarchical clustering', total=len(sentences_ids)):
