...
def stage_1():
    ...
    #print(json.dumps({k: list(v) for k,v in labels_by_sources.items()}, indent=2))
    #exit(0)
                ...
                if claim_urls:
                    ...
                    if score != None:
                        ...
                        if score >= 0.8:
                            label = 'true'
                    if label:
                        results.append({'url': claim_urls, 'label': label, 'source': 'datacommons_factcheck'})

                    #results.append({'url': 'a'})
                claimReviews['{}::{}'.format(fixed_url, j)] = claimReview
