...
def process(data, output_folder, source):
    ...
    for domain, props in data.items():
        ...
        # print(looking_at)
        classes = set(
            [a for a in [claimreview.simplify_label(el) for el in looking_at] if a]
        )
        if len(classes) != 1:
...
def main(which="opensources"):
    if which == "opensources":
        data_opensources = utils.read_json(
            subfolder_opensources / "source" / "sources" / "sources.json"
        )
        process(data_opensources, subfolder_opensources, "opensources")
    if which == "melissa_zimdars":
        data_zimdars_tsv = utils.read_tsv(
            subfolder_zimdars / "source" / "melissa_zimdars_spreadsheet.tsv"
        )
