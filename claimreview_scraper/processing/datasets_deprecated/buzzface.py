...
def main():
    ...
    data = {
        el["post_id"]: {"url": el["Post URL"], "label": el["Rating"]}
        for el in source_data
        if el["Post Type"] == "link"
    }
    ...
    for id, el in data.items():
        ...
        if label_binary:
            results.append(
                {"url": el["url"], "label": label_binary, "source": "buzzface"}
            )
        file_path = folder / "intermediate" / "{}.html".format(id)
        if not os.path.isfile(file_path):
            response = requests.get(el["url"])
            utils.write_file_with_path(
                response.text, folder / "intermediate", "{}.html".format(id)
            )
    ...
    for file_location in glob.glob(str(folder / "intermediate/*.html")):
        ...
        fb_urls = re.findall(
            r'<a\shref="([^>]*)" tabindex="-1" target="_blank"', string
        )
        real_urls = [
            urlparse.parse_qs(urlparse.urlparse(u).query)["u"] for u in fb_urls
        ]
