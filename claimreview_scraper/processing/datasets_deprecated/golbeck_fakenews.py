...
def main():
    ...
    urls = [
        {"url": row["URL of article"], "label": "fake", "source": "golbeck_fakenews"}
        for row in data
        if row["Fake or Satire?"].strip() == "Fake"
    ]
    ...
    for el in data:
        claimReview = {
            ...
            "url": el,
            "author": {"@type": "Person", "url": verifier_url},
            "claimReviewed": "",
            "reviewRating": {
                "@type": "Rating",
                "ratingValue": -1,
            ...
            "itemReviewed": {"@type": "CreativeWork", "sameAs": el["URL of article"]},
        }
    ...
    rebuttals = {
        el["URL of article"]: {
            u.strip(): ["golbeck_fakenews"]
            for u in el["URL of rebutting article"].split("; ")
        }
        for el in data
    }
