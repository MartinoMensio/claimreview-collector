...
def main():
    ...
    urls = [
        {
            "url": clean_url(el["url"]),
            "label": claimreview.simplify_label(el["type"]),
            "source": "pontes_fakenewssample",
        }
        for el in data
        if el["type"]
    ]
    ...
    domains = {
        k: set([l["label"] for l in g])
        for k, g in itertools.groupby(
            sorted(domains, key=lambda el: el["domain"]), key=lambda el: el["domain"]
        )
    }
    ...
    domains = [
        {"domain": k, "label": el.pop(), "source": "pontes_fakenewssample"}
        for k, el in domains.items()
    ]
