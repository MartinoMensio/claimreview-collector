...
def aggregate_initial():
        ...
        if config['rebuttals']:
            ...
            for source_url, rebuttal_l in rebuttals.items():
                for rebuttal_url, source in rebuttal_l.items():
                    all_rebuttals[source_url] = merge_rebuttals(all_rebuttals.get(
                        source_url, []), {'url': rebuttal_url, 'source': source})
    ...
    print('before aggregation #urls', urls_cnt, ': fake',
          fake_urls_cnt, 'true', urls_cnt - fake_urls_cnt)
    print('before aggregation #domains', domains_cnt, ': fake',
          fake_domains_cnt, 'true', domains_cnt - fake_domains_cnt)
    ...
    print('len aggregated fact_checking_urls',
          len(aggregated_fact_checking_urls))
...
def check_and_add_url(new_url, new_label, new_sources, aggregated_urls):
    ...
    if match:
        ...
        if label != new_label:
            raise ValueError(
                'labels differ: old {} new {}'.format(label, new_label))
        sources += new_sources
    else:
        label = new_label
        sources = new_sources
